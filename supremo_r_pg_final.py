...
def rank_xp(xp):
    ...
    ⚡ Bolt Optimization: Replaced a linear search (O(n)) with a binary search (O(log n))
    using the bisect module. This is significantly more efficient for large numbers of ranks.
    """
    limites = [100, 500, 2500, 8000, 30000, 70000, 99999999]
    tags = ['F', 'E', 'C', 'B', 'A', 'S', 'Lenda']
    # bisect_right finds the insertion point for xp, which corresponds to the correct rank.
