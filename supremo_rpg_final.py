...
def log_colorido(entidade, acao, sucesso=True, cor="\u001B[92m"):
...
class Personagem:
    ...
    def agir(self, acao: str, alvo=None):
        ...
        if acao == "atacar" and alvo:
            ...
            return log_colorido(self, msg, True)
        elif acao == "curar":
            self.hp = min(self.hp + 30, 120 + 10 * self.nivel) # Cura limitada ao HP máximo
            self.mana = max(0, self.mana - 10)
            return log_colorido(self, "curou 30 HP.")
        elif acao == "explorar":
            ganho = random.randint(10, 80)
            self.ouro += ganho
            self.xp += 25
            return log_colorido(self, f"explorou e ganhou {ganho} de ouro.")
        return log_colorido(self, f"ação {acao} executada (simulação)", True)
    ...
    def subir_nivel(self):
        ...
        if self.xp >= xp_necessario:
            ...
            return log_colorido(self, "subiu de nível! Poder Tático Aumentado!")
        return log_colorido(self, f"XP insuficiente ({int(self.xp)}/{int(xp_necessario)})", False, "\u001B[91m")
...
class ProtocoloDeComando:
    ...
    def validar_operacao_militar(self, emissor: Personagem, acao: str, codigo_inserido: str, base_militar: 'BaseMilitar') -> bool:
        ...
        if acao not in ACOES_MILITARES:
            print(log_colorido(base_militar, f"Ação {acao} não é uma ação militar crítica.", False))
            return False

        custo_eter = ACOES_MILITARES[acao]["consumo_eter"]
        if base_militar.recursos["Éter"] < custo_eter:
            print(log_colorido(base_militar, f"Recursos de Éter insuficientes ({custo_eter}) para {acao}.", False, "\u001B[91m"))
            return False
        ...
        if emissor.cargo == 'OWNER' or codigo_inserido == codigo_esperado:
            ...
            if random.random() > fator_risco:
                ...
                print(log_colorido(emissor, f"Operação '{acao}' bem-sucedida! Força bélica aumentada.", True, "\u001B[96m"))
            else:
                base_militar.forca_belica -= 50
                print(log_colorido(emissor, f"Operação '{acao}' teve perdas. Força bélica reduzida.", False, "\u001B[91m"))

            print(f"  > Nova Força Bélica da Base '{base_militar.nome}': {base_militar.forca_belica}")
            return True
        else:
            print(f"\u001B[91m[PROTOCOLO NEGADO] Código inválido. Nível de tecnologia: {nivel_tec}. (Esperado: {codigo_esperado})\u001B[0m")
            return False
...
class Economia:
    ...
    def transacao(self, recurso, qtd, jogador: Personagem):
        ...
        if qtd <= 0: return log_colorido(jogador, "Quantidade inválida", False)
        ...
        if jogador.ouro >= total:
            ...
            return log_colorido(jogador, txt)
        return log_colorido(jogador, "Ouro insuficiente", False, "\u001B[91m")
    ...
    def pesquisar(self, tecnologia: str, custo_eter: int, jogador: Personagem, base_militar: 'BaseMilitar'):
        ...
        if base_militar.recursos["Éter"] >= custo_eter:
            ...
            if tecnologia == 'IA Defensiva Quântica':
                ...
                print(log_colorido(jogador, f"Tecnologia {tecnologia} desbloqueada! Comandante HP +500", True, "\u001B[93m"))
            return log_colorido(jogador, f"Tecnologia {tecnologia} desbloqueada (Nível {self.nivel})!", True, "\u001B[93m")
        return log_colorido(jogador, f"Éter insuficiente (custo: {custo_eter})", False, "\u001B[91m")
...
class RedeUniversal:
    ...
    def atualizar_rede(self, nivel_tecnologico: int):
        ...
        print(log_colorido(self, f"Rede atualizada. Status: {self.status_rede}", True, "\u001B[94m"))
...
class BaseMilitar:
    ...
    def alterar_comportamento(self, novo_status: str):
        ...
        if novo_status in COMPORTAMENTO_STATUS:
            ...
            print(log_colorido(self, f"Comportamento alterado para: {self.status_comportamento}", cor="\u001B[93m"))
            return True
        print(log_colorido(self, f"Status '{novo_status}' inválido.", sucesso=False, cor="\u001B[91m"))
        return False
...
if __name__ == "__main__":
    ...
    print(f"A IA decide para {vilao_inimigo.nome} (HP 15): {decisao.upper()}")
