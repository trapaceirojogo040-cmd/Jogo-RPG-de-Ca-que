...
# O foco é na interdependência dos módulos: a tecnologia afeta o protocolo e a economia.
...
class ServicoDeArmazenamento:
    ...
    def execute_entropy_protocol(self, max_days_inactive: int = 30):
        ...
        for user_id in keys_to_destroy:
            ...
            print(f" [PROTOCOLO DE ENTROPIA]: Dados de {user_id[:8]} destruídos por inatividade.")
...
PHRASES_MILITARES: Dict[str, str] = {
    ...
    "RECUO_ESTRATEGICO": "Recuar é prioridade, a força retornará.",
...
# 3. --- OBJETOS DO JOGO (NÚCLEO DO RPG) ---
class Personagem:
    ...
    def agir(self, acao: str, alvo=None):
        ...
        if acao == "atacar" and alvo:
            ...
            return frase_log(self, "curou 30 PV.")
        elif acao == "explorar":
    ...
    def subir_nivel(self):
        ...
        if self.xp >= xp_necessario:
            ...
            self.historico.append(f"Subiu para o nível {self.nivel} (Patente {self.rank})")
...
class ProtocoloDeComando:
    ...
    def validar_operacao_militar(self, emissor: Personagem, acao: str, codigo_inserido: str, base_militar: 'BaseMilitar') -> bool:
        ...
        if acao not in ACOES_MILITARES:
            print(frase_log(base_militar, f"Ação {acao} não é uma ação militar crítica.", False))
            return False

        custo_eter = ACOES_MILITARES[acao]["consumo_eter"]
        if base_militar.recursos["Éter"] < custo_eter:
            print(frase_log(base_militar, f"Recursos de Éter insuficientes ({custo_eter}) para {acao}.", False, "\u001B[91m"))
            return False
        ...
        if emissor.cargo == 'PROPRIETÁRIO' or codigo_inserido == codigo_esperado:
            ...
            if random.random() > fator_risco:
                ...
                print(frase_log(emissor, f"Operação '{acao}' bem-sucedida! Força de combate aumentada.", True, "\u001B[96m"))
            else:
                base_militar.forca_belica -= 50
                print(frase_log(emissor, f"Operação '{acao}' sofreu perdas. Força de combate reduzida.", False, "\u001B[91m"))

            print(f"  > Nova Força de Combate da Base '{base_militar.nome}': {base_militar.forca_belica}")
            return True
        else:
...
class Economia:
    ...
    def transacao(self, recurso, qtd, jogador: Personagem):
        ...
        if qtd <= 0: return frase_log(jogador, "Quantidade inválida.", False)
        ...
        if jogador.ouro >= total:
            ...
            txt = f"comprou {qtd} de {recurso} por {total} de Ouro. Preço/Unidade: {preco_final}"
...
class BaseMilitar:
    ...
    def alterar_comportamento(self, novo_status: str):
        ...
        if novo_status in COMPORTAMENTO_STATUS:
            ...
            print(frase_log(self, f"Comportamento da base alterado para: {self.status_comportamento}", cor="\u001B[93m"))
            return True
...
class IA_NPC:
    ...
    def decidir_acao_npc(self, npc: Personagem, alvo: Personagem) -> str:
        ...
        if scores["cura"] > 0.6: return "cura" # Prioridade alta para cura se a saúde estiver < ~40%
...
if __name__ == "__main__":
    ...
    decisao = ai.decidir_acao_npc(vilao_inimigo, proprietario) # A IA decide a ação do vilão
