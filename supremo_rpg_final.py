...
class Cores:
    """Refatoração de ANSI escape codes para melhor legibilidade."""
    RESET = "\u001B[0m"
    VERMELHO = "\u001B[91m"
    VERDE = "\u001B[92m"
    AMARELO = "\u001B[93m"
...
PROPRIETARIO_CONTA = ContaUsuario("caiquesanto674@gmail.com", SENHA_BASE, "PROPRIETÁRIO")

# 2. --- FEEDBACK EM COR (LOGS E INTERFACE) ---
...
def exibir_painel(titulo: str, dados: Dict[str, Any], nivel: int = 0):
    """
    Exibe dados formatados com padding dinâmico e cores para melhor UX no CLI.
    🛡️ SENTINEL: Garante que dados vazios não quebrem a interface.
    """
    if not dados:
        if nivel == 0: print(f"\n{Cores.MAGENTA}=== {titulo.upper()} ==={Cores.RESET}")
        print(f"{'  ' * (nivel + 1)}{Cores.CINZA}(vazio){Cores.RESET}")
        if nivel == 0: print(f"{Cores.MAGENTA}{'=' * (len(titulo) + 8)}{Cores.RESET}")
        return

    if nivel == 0:
        print(f"\n{Cores.MAGENTA}=== {titulo.upper()} ==={Cores.RESET}")

    # Padding dinâmico baseado na maior chave
    max_label = max(len(str(k)) for k in dados.keys())

    for chave, valor in dados.items():
        indent = "  " * (nivel + 1)
        label = f"{chave}:".ljust(max_label + 2)

        if isinstance(valor, dict):
            print(f"{indent}{Cores.CIANO}{label}{Cores.RESET}")
            exibir_painel("", valor, nivel + 1)
        else:
            # 🎨 Palette: Cores temáticas para diferentes tipos de dados
            if isinstance(valor, bool):
                valor_str = f"{Cores.VERDE}Sim{Cores.RESET}" if valor else f"{Cores.VERMELHO}Não{Cores.RESET}"
            elif any(x in chave for x in ["PV", "Bélica", "Risco"]):
                valor_str = f"{Cores.VERMELHO}{valor}{Cores.RESET}"
            elif "Ouro" in chave or "Recursos" in chave:
                valor_str = f"{Cores.AMARELO}{valor}{Cores.RESET}"
            elif any(x in chave for x in ["XP", "Nível", "Patente", "Poder"]):
                valor_str = f"{Cores.VERDE}{valor}{Cores.RESET}"
            elif "Status" in chave:
                valor_str = f"{Cores.AZUL}{valor}{Cores.RESET}"
            else:
                valor_str = str(valor)

            print(f"{indent}{Cores.CIANO}{label}{Cores.RESET} {valor_str}")
