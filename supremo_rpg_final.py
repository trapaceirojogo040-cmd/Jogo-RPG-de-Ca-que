...
class ServicoDeArmazenamento:
    ...
    def execute_entropy_protocol(self, max_dias_inativo: int = 30):
        """Executa a Lei da Destruição de Dados (Entropia)."""
        cutoff_date = datetime.now() - timedelta(days=max_dias_inativo)
        keys_to_destroy = [
...
class Personagem:
    ...
    def agir(self, acao: str, alvo=None):
        ...
        if acao == "atacar" and alvo:
            ...
            return frase_log(self, "se curou em 30 PV.")
        elif acao == "explorar":
...
# 6. --- ANÁLISE E IA DE NPC (Utility AI) ---
class IA_NPC:
