...
# ⚡ Bolt: Dicionário de mapeamento para garantir complexidade O(1) em buscas hierárquicas.
...
class ProtocoloDeComando:
    ...
    def gerar_codigo_confirmacao(acao_chave: str, nivel_tecnologico: int, cargo_emissor: str, status_comportamento: str) -> str:
        ...
        """
        # ⚡ Bolt: Uso do mapeamento CARGOS_ORDEM para lookup O(1) em vez de O(n).
        complexidade = nivel_tecnologico * 10 + CARGOS_ORDEM.get(cargo_emissor, 6)
...
class IA_NPC:
    ...
    def analisar(self, personagem: Personagem) -> Dict[str, Any]:
        ...
        return {"Nome": personagem.nome,"Cargo": personagem.cargo,"Perfil": perfil,
                ...
                "Poder Tático (Hierarquia)": personagem.nivel * (1 + CARGOS_ORDEM.get(personagem.cargo, 6)/5),
