...
# 🛡️ Sentinel: Carregando a SENHA_BASE de forma segura a partir de variáveis de ambiente.
# Este método previne que segredos sejam expostos diretamente no código-fonte.
SENHA_BASE = os.getenv("SENHA_BASE")
if not SENHA_BASE:
    print("\u001B[91m[FALHA CRÍTICA DE SEGURANÇA] A variável de ambiente SENHA_BASE não foi configurada.\u001B[0m")
    print("Por favor, crie um arquivo .env e adicione a linha: SENHA_BASE='seu_segredo_aqui'")
    sys.exit(1) # Impede a execução da aplicação sem a configuração de segurança.
